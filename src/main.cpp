...
static uint8_t u8TxBuffer[64] = {10,20,30,40,50,60};
static uint8_t u8RxBuffer[64] = {0};
extern wifi_mode_t mode;
...
*/
  ...
  for (uint16_t u16Id = 0; u16Id < 64; u16Id++)
  {
    u8TxBuffer[u16Id] = 10 + u16Id;
  }

  writeMem(&u8TxBuffer[0], 0x7A, 33);
  readMem(&u8RxBuffer[0], 0x7A, 33);

  bool bRead_ok = false;

  for (uint16_t u16Index = 0; u16Index < 33; u16Index++)
  {
    if (u8RxBuffer[u16Index] != u8TxBuffer[u16Index])
    {
      bRead_ok = false;
      break;
    }
  ...
  if (bRead_ok == true)
  {
    bRead_ok = false;
    Serial.printf("\n");
    Serial.printf("Read operation successful\n");
    Serial.printf("data read  : | ");
    for (uint8_t u8Index = 0; u8Index < 33; u8Index++)
    {
        Serial.printf("%d", u8RxBuffer[u8Index]);
        Serial.printf(" | ");
    }
    Serial.printf("\n");

    Serial.printf("data write : | ");
    for (uint8_t u8Index = 0; u8Index < 33; u8Index++)
    {
        Serial.printf("%d", u8TxBuffer[u8Index]);
        Serial.printf(" | ");
    }
    Serial.printf("\n\n");
  }
  ...
  {
    Serial.printf("\n");
    Serial.printf("Read operation fail\n");
    Serial.printf("data read  : | ");
    for (uint8_t u8Index = 0; u8Index < 33; u8Index++)
    {
        Serial.printf("%d", u8RxBuffer[u8Index]);
  ...
  // Serial.printf("Buffer write: %s\r\n",MACtoString(u8TxBuffer));
