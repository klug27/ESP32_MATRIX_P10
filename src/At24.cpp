...
void i2cInit(void)
{
  if (i2cInitialized == false)
  {
    ...
    i2cInitialized = Wire.setPins(SDA_PIN,SCL_PIN);

    if (i2cInitialized == true)
    {
      Serial.println("Successful configured I2C pins");
    }
    else
    {
      Serial.println("fail to configured I2C pins");
      while (1);
    }
    ...
    // set flag
    // i2cInitialized = true;
    i2cInitialized = Wire.begin();

    if (i2cInitialized == true)
    {
      Serial.println("Successful initialized I2C driver");
    }
    else
    {
      Serial.println("fail to initialized I2C driver");
      while (1);
    }
  }
}
...
void Config_eeprom(void)
{
  if (i2cInitialized == true)
  {
    ...
    {
      Serial.println("Sucessfully communicated with AT24 memory");
      delay(5);
